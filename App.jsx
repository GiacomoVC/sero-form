...
import { WORLDS, SUBCATEGORIES, DATES, VENUE, PRICE, YAPE_NUMBER, WHATSAPP_NUMBER, SHEETS_URL } from "./data";
import "./styles.css";
...
export default function App() {
  ...
  const waMessage = useMemo(() => {
    ...
    // === DATA line (paste this to build tables) ===
    const dataLine = `SERO|${data.nombre}|${data.whatsapp}|obs:${obsParts.join("·") || "-"}|${fechasShort}|${mas1}|${estiloShort}|edad:${data.edad}|com:${comShort}`;

    // === readable version (for your own eyes) ===
    const lines = [];
    lines.push(dataLine);
    lines.push("—");
    lines.push("🎴 *Reserva Sero*");
    lines.push(`*Nombre:* ${data.nombre}`);
    lines.push(`*WhatsApp:* ${data.whatsapp}`);
    if (data.referido) lines.push(`*Dato de:* ${data.referido}`);
    if (tasteParts.length) {
      lines.push("*Gustos:*");
      chosenWorlds.forEach((w) => {
        if (w.id === "otro") { if (data.otroMundo) lines.push(`  Otro mundo: ${data.otroMundo}`); return; }
        const subs = (data.subs[w.id] || []).map((sid) => SUBCATEGORIES[w.id].find((s) => s.id === sid)?.label).filter(Boolean);
        const tits = [...(data.titulos[w.id] || [])];
        const otroT = data.otro[w.id];
        if (otroT) tits.push(`${otroT} (otro)`);
        const obs = data.obsesion[w.id];
        if (subs.length || tits.length) {
          let l = `  ${w.emoji} ${w.label}: ${subs.join(", ")}`;
          if (tits.length) l += ` → ${tits.join(", ")}`;
          lines.push(l);
          if (obs) lines.push(`     🔥 obsesión #1: ${obs}`);
        }
      });
    }
    lines.push(`*Fechas:* ${data.fechas.join(" / ") || "(ninguna)"}`);
    lines.push(`*+1:* ${data.masuno === "amigo" ? `Sí — ${data.nombreMasuno || "(sin nombre)"}` : "Va solo"}`);
    lines.push(`*En la mesa:* ${data.estilo}`);
    lines.push(`*Edad:* ${data.edad}`);
    lines.push(`*Comida:* ${comShort === "ok" ? "Sin restricciones" : com.join(", ")}`);
    lines.push("");
    lines.push("_Ya hice el Yape, adjunto la captura 👇_");
    ...
    <div className="wrap">
      ...
      {STEPS[step] === "bienvenida" && (
        <section className="screen center">
          ...
          <p className="lead">Una mesa de gente fan de lo mismo que tú. Llena esto en 2 minutos y te armamos tu plan. ✨</p>
      ...
      {STEPS[step] === "mesa" && (
        <section className="screen">
          <h2>🗣️ ¿Cómo eres en una mesa con gente nueva?</h2>
      ...
      {STEPS[step] === "edad" && (
        <section className="screen">
          ...
          <button className={`opt ${data.masuno === "amigo" ? "on" : ""}`} onClick={() => update({ masuno: "amigo" })}>Voy con un amig@ 👯</button>
          {data.masuno === "amigo" && (
      ...
      {STEPS[step] === "comida" && (
        <section className="screen">
          <h2>🍽️ ¿Alguna restricción alimentaria?</h2>
          <div className="grid">
      ...
      {STEPS[step] === "yape" && (
        <section className="screen center">
          ...
          <div className="card">
            <p>🍕 La cena es en <strong>{VENUE}</strong>.</p>
            <p>💰 <strong>{PRICE}</strong> — incluye pizza artesanal y bebida.</p>
            <p>Apartas tu lugar con un Yape a <strong>{YAPE_NUMBER}</strong>.</p>
            <p className="hint">Al tocar el botón te abrimos WhatsApp con tus datos. Mándanoslos junto con la captura del Yape y quedas dentro. 👇</p>
      ...
      {STEPS[step] === "final" && (
        <section className="screen center">
          <div className="badge">🎴</div>
          <h2>¡Listo!</h2>
          <p className="lead">Te confirmamos tu mesa por WhatsApp. Prepárate para caer a tu plan. 🙌</p>
        </section>
      )}
    </div>
  );
...
function DetailScreens({ worlds, data, update, toggle, onBack, onNext }) {
  ...
  return (
    <section className="screen" key={w.id}>
      ...
      {showObsesion && (
        <label className="mt">🔥 De todo lo que marcaste, ¿cuál es tu obsesión #1 ahorita?
